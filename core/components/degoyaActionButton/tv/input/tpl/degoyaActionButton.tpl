<span onclick="btnAction('{$id}','{$tv->get('value')|escape}','{$saveResource}')" id="tv-actionbutton-{$tv->id}" class="x-btn x-btn-small x-btn-icon-small-left x-btn-degoyaactionbutton x-btn-noicon" data-value="{$tv->get('value')|escape}" unselectable="on">{$buttonCaption} {$saveResource}</span>
...
{literal}
    ...
    function btnAction( resource, snippet, saveresource) {
